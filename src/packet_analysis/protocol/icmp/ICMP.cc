...
void ICMPAnalyzer::ContinueProcessing(Connection* c, double t, bool is_orig, int remaining, const Packet* pkt)
	...
	int data_len = pkt->ip_hdr->PayloadLen();
	...
	if ( packet_contents && len > 0 )
		...
		conn->EnqueueEvent(packet_contents, nullptr, conn->GetVal(), std::move(contents));
		}
	...
	if ( zeek::detail::rule_matcher )
		...
		if ( ! matcher_state.MatcherInitialized(is_orig) )
			// TODO: passing null here probably breaks stuff
			matcher_state.InitEndpointMatcher(nullptr, ip.get(), len, is_orig, nullptr);
		}
	...
	if ( ip->NextProto() == IPPROTO_ICMP )
		NextICMP4(run_state::current_timestamp, icmpp, len, caplen, data, ip.get());
	else if ( ip->NextProto() == IPPROTO_ICMPV6 )
		NextICMP6(run_state::current_timestamp, icmpp, len, caplen, data, ip.get());
	else
		{
	...
	// TODO: replace this with packet_analysis forwarding
	// if ( caplen >= len )
...
void ICMPAnalyzer::ICMP_Sent(const struct icmp* icmpp, int len, int caplen,
    ...
    {
	if ( icmp_sent )
		conn->EnqueueEvent(icmp_sent, nullptr,
			conn->GetVal(),
	...
	if ( icmp_sent_payload )
		...
		conn->EnqueueEvent(icmp_sent_payload, nullptr,
			conn->GetVal(),
			BuildInfo(icmpp, len, icmpv6, ip_hdr),
...
void ICMPAnalyzer::Describe(ODesc* d) const
	{
...
void ICMPAnalyzer::UpdateConnVal(zeek::RecordVal *conn_val)
	{
...
void ICMPAnalyzer::UpdateEndpointVal(const ValPtr& endp_arg, bool is_orig)
	{
